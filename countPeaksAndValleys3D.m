function [ valleypeakIndices2,totalPeakNum] = countPeaksAndValleys3D(data, windowSize,proximityThreshold)
...
for idx = 1:(M * (M + 1) / 2)
    ...
    peakExtremaIndices(2:n-1) = peakExtremaIndices(2:n-1) | isPeak ;
end
...
for k = 1:n
    ...
    if peakExtremaIndices(k)
        if lastIndex2 == 0
            valleypeakIndices2=[valleypeakIndices2;k];
            lastIndex2 = k;
        elseif (k - lastIndex2) > 30%proximityThreshold
             valleypeakIndices2=[valleypeakIndices2;k];
             lastIndex2 = k;
        end
    end
