function [SER,rmserr,bigHfit,opts]=VFdriver(bigH,s,poles,opts,F,valleypeak)
...
opts.poletype='newlzqlx';%%%%%效果不算太显著
...
if isempty(poles)
    ...
    if length(opts.poletype)==8
        ...
        if opts.poletype=='lincmplx' %Complex, linearly spaced starting poles
            bet = linspace(s(1)/1i, s(Ns)/1i, floor(N/2));
            poles = zeros(1, 2 * length(bet)); % 预分配内存
            for n = 1:length(bet)
                alf = -nu * bet(n);
                poles(2*n-1) = alf - 1i * bet(n); % 存储第一个极点
                poles(2*n) = alf + 1i * bet(n);   % 存储第二个极点
            end
        elseif opts.poletype=='logcmplx' %Complex, logaritmically spaced starting poles
            bet=logspace(log10(s(1)/1i),log10(s(Ns)/1i),floor(N/2));
            poles=[];
            for n=1:length(bet)
                alf=-nu*bet(n);
             ...
             elseif opts.poletype=='newlzqlx' %Complex, linearly spaced starting poles 
      %%%%%%%%improve
      fprintf('run our initial fuction\n')
      s2 = s;
%       bet=linspace(s2(1)/i,s2(Ns)/i,floor(N/2));

     if length(valleypeak)>floor(N/2)
        [bet,maxValues] = maxk(valleypeak, floor(N/2));
     ...
     if length(valleypeak)<floor(N/2)
         bet=linspace(s2(2)/i,s2(Ns)/i,floor((N-length(valleypeak)*2)/2));
         bet=[bet,valleypeak'];
     end
    ...
    if length(poles)<N %An odd number of poles was prescribed
        if strcmp(opts.poletype,'lincmplx')| strcmp(opts.poletype,'newlzqlx') 
...
tell=0;
f = zeros(Nc*(Nc+1)/2, size(bigH,3)); % 预分配f的大小
for col=1:Nc
    for row=col:Nc
        tell=tell+1;
        f(tell,:)=reshape(bigH(row, col, :), [1, size(bigH,3)]); %stacking elements into a single vector
    end
end
nnn=tell;
    ...
    for row=1:Nc
        for col=row:Nc
            ...
            elseif weightparam==3
                f_sum=f_sum+f(tell,:)/sqrt(norm(f(tell,:)));
            end
            %end
        end
    end
...
if isempty(opts.weight) %Automatic specification of weight
    ...
    for row=1:Nc
        for col=row:Nc
            tell=tell+1;
            weight(tell,:)=reshape(opts.weight(row, col, :), 1, Ns);
        end
    end
...
threshold = 0.003;     % 设定1%的阈值
circle_threshold=0.0003;
...
if Nc>1 %Will do only for multi-terminal case
    ...
    for iter=1:Niter1
        ...
        if iter == 1
            % 第一次计算 currentValue
            SER2 = tri2full(SER);
            currentValue = norm_error(SER2,F,reshape(f_sum, 1, 1, Ns),fit);  % 计算当前的值, 替换成实际的函数或计算逻辑
...
for iter=1:Niter2
    ...
    [SER,poles,~,fit1]=vectfit3(f,s,poles,weight,VF);
    if iter == 1
        % 第一次计算 currentValue
        SER2 = tri2full(SER);
        currentValue = norm_error(SER2,F,bigH,fit1);  % 计算当前的值, 替换成实际的函数或计算逻辑
        previousValue = currentValue;    % 更新previousValue
        continue;  % 第一次不进行比较，跳过剩下逻辑
    end
    if mod(iter, 5) == 0
...
if remove_HFpoles==1
    ...
    [SER,poles,~,fit2]=vectfit3(fit1,s,poles,weight,VF);
end
%===========================================
