...
currenterrValue = 0.1;
previouserrValue = 0.1;
threshold = 0.001;     % 设定阈值
...
<<<<<<< HEAD
    ...
    time_iter = toc; % 结束计时
    %fprintf('迭代循环计算时间: %.4f 秒 currentValue: %.15f\n', time_iter,currentValue); % 输出迭代循环计算时间
%     for N_error = N:2:N     
%         tic; % 开始计时
%         bet = linspace(s(1)/1i, s(Ns)/1i, floor(N_error/2));
%         poles_new = zeros(1, 2 * length(bet)); % 预分配内存
%         for n = 1:length(bet)
%             alf = -nu * bet(n);
%             poles_new(2*n-1) = alf - 1i * bet(n); % 存储第一个极点
%             poles_new(2*n) = alf + 1i * bet(n);   % 存储第二个极点
%         end
%         for iter=1:Niter1
%             if opts.screen==1
%                 disp(['   Iter ' num2str(iter)])
...
%                 VF.skip_res = 1;
%             end
%             [SER,poles_new,rmserr,fit]=vectfit3(f_sum,s,poles_new,weight_sum,VF);
%             if iter == 1
%                 % 第一次计算 currentValue
%                 SER2 = tri2full(SER);
%                 currentValue = norm_error(SER2,F,reshape(f_sum, 1, 1, Ns));  % 计算当前的值, 替换成实际的函数或计算逻辑
...
%                 % 检查相差是否不超过1%%
%                 if relativeDifference <= threshold && currentValue<0.10
%                     fprintf('在 极点数量为%d 的iter1 = %d 时，相差不超过 1%%%%，停止循环1。\n', N_error,iter);
%                     break;
%                 end
%                 % 更新 previousValue 为当前值
%                 previousValue = currentValue;
...
%         end
%         time_iter = toc; % 结束计时
%         fprintf('迭代循环计算时间: %.4f 秒\n', time_iter); % 输出迭代循环计算时间
%         currenterrValue = currentValue;
%         if N_error~=N
%             if(currenterrValue*(N_error+2))>(previouserrValue*N_error)
%                 break;
%             end
%         end
%         poles = poles_new;
%         previouserrValue = currenterrValue;
%     end
