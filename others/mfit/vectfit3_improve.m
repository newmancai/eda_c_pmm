function [SER,poles,rmserr,fit,opts]=vectfit3_improve(f,s,poles,weight,opts,SER);
%
...
%     
%     new_pole=[SERA;poles];
%     new_poles1 = new_pole(imag(new_pole) == 0);
%     new_poles2 = new_pole(imag(new_pole) ~= 0);
%     
%     new_residue=[abs(C(:));abs(SER.C(:))];
%     new_residues1 = new_residue(imag(new_pole) == 0);
%     new_residues2 = new_residue(imag(new_pole) ~= 0);
%     sums = new_residues2(1:2:end)+new_residues2(2:2:end); % 每一对求和
%     
%     
%     half_length_real = floor(length(SERA(imag(SERA) == 0)));
%     [~, sorted_indices] = sort(new_residues1, 'descend');
%     top_half_indices_real = sorted_indices(1:half_length_real);
%     top_half_values_real = new_residues1(top_half_indices_real);
%     top_half_poles_real = new_poles1(top_half_indices_real);
%     
%     
%     half_length_imag = floor(length(SERA(imag(SERA) ~= 0))/2);
%     [~, sorted_indices_imag] = sort(sums, 'descend');
%     top_half_indices_imag = sorted_indices_imag(1:half_length_imag);
%     
%     positions = [2*top_half_indices_imag;2*top_half_indices_imag-1];
%     [sorted_positions, sort_order] = sort(positions);
%     
%     new_A=[top_half_poles_real;new_poles2(sorted_positions)];
%     new_C=[top_half_values_real;new_residues2(sorted_positions)];
%     
%     SERA=new_A;
%     SERC=C;
%    end
