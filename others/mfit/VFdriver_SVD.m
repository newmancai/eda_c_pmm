function [SER]=VFdriver_SVD(bigH,s,poles,opts,F,valleypeak)
...
%       bet=linspace(s2(1)/i,s2(Ns)/i,floor(N/2));
     ...
     if length(valleypeak)<floor(N/2)
         bet=linspace(s2(2)/1i,s2(Ns)/1i,floor((N-length(valleypeak)*2)/2));
         bet=[bet,valleypeak'];
     end
...
if Nc>1 %Will do only for multi-terminal case
    ...
    for iter=1:Niter1
        ...
        if iter == 1
            % 第一次计算 currentValue
            %SER2 = tri2full(SER);
            currentValue = norm_error(SER,F,reshape(f_sum, 1, 1, Ns),fit);  % 计算当前的值, 替换成实际的函数或计算逻辑
